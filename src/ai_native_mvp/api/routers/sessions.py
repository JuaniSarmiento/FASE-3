...
async def update_session(
    ...
    response_data = SessionResponse(
        ...
        start_time=db_session.start_time,
        end_time=db_session.end_time,
        trace_count=len(db_session.traces) if hasattr(db_session, "traces") else 0,
        risk_count=len(db_session.risks) if hasattr(db_session, "risks") else 0,
        created_at=db_session.created_at,
        updated_at=db_session.updated_at,
    )
...
async def delete_session(
    ...
    try:
        ...
        db.rollback()
        # Re-lanzar como DatabaseError para manejo consistente
...
async def get_session_history(
    ...
    **Agregaciones incluidas**:
    ...
    if status:
        query = query.where(SessionDB.status == status.upper())
