...
class ResponseGenerator:
    ...
    def _generate_simulator_response(
        ...
        # Generate response
        response = simulator.simulate(prompt, context or {})
    ...
    def _get_simulator(self, simulator_type: str):
        ...
        if simulator_type not in self._simulators:
            if simulator_type == "PO":
                self._simulators["PO"] = ProductOwnerSimulator(self.llm_provider)
            elif simulator_type == "SM":
                self._simulators["SM"] = ScrumMasterSimulator(self.llm_provider)
            elif simulator_type == "IT":
                self._simulators["IT"] = TechnicalInterviewerSimulator(self.llm_provider)
            else:
